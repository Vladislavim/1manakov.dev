...
export function CaseEvidence({slug}:{slug:string}){
 ...
 return <section className={`case-evidence evidence-${evidence.layout}`} aria-labelledby="evidence-heading">
  ...
  <div className="evidence-pair">{[{src:evidence.before,label:'PREVIOUS VERSION',number:'01'},{src:evidence.after,label:'IMPLEMENTED REDESIGN',number:'02'}].map(item=><figure key={item.number}><figcaption><span>{item.label}</span><span>{item.number}</span></figcaption><a href={item.src} target="_blank" rel="noreferrer" aria-label={`Open full-size ${item.label.toLowerCase()}`}><Image src={item.src} alt={`${slug} — ${item.label.toLowerCase()}`} width={1600} height={900} sizes="(max-width:767px) 92vw, 85vw"/></a></figure>)}</div>
