...
export function Explore() {
  ...
  return <section id="explore" ref={root} className="explore scene" aria-labelledby="explore-title">
    <div className="explore-inner">
      ...
      <div className="scene-index" aria-label="Explore featured work">{featuredProjects.map((project, i) => <button key={project.slug} aria-label={`Preview ${project.name}`} aria-pressed={active === i} onClick={() => setActive(i)}>0{i + 1}<span /></button>)}</div>
