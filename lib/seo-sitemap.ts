...
export function sitemapGroups(){
 ...
 const paths=['/','/about','/lab',...projects.map(p=>`/work/${p.slug}`),...(guides.length?['/guides','/guides/editorial-policy',...pages('/guides',guides.length),...topics.flatMap(t=>[`/guides/topic/${t}`,...pages(`/guides/topic/${t}`,guides.filter(g=>g.topic===t).length)])]:[])];
