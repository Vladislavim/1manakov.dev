...
test('home, case portal, history and navigation', async ({ page }) => {
  ...
  await expect(page.locator('.position-0')).toHaveAttribute('data-project', 'pdp');
  await page.locator('.position-0').click();
  await expect(page).toHaveURL(/work\/pdp/);
  await expect(page.getByRole('heading', { name: 'PDP', exact: true })).toBeVisible();
...
test('five experiments and keyboard dismissal', async ({ page }) => {
  await page.goto('/lab');
  for (const name of ['Cursor reveal', 'Card physics', 'Type morph', 'Portal', 'Grid shift']) {
    await page.getByRole('button', { name: new RegExp(name) }).click();
    await expect(page.getByRole('dialog')).toBeVisible();
    await expect(page.getByRole('dialog').getByRole('heading', { name: new RegExp(name) })).toBeVisible();
    await expect(page.locator('.lab-surface')).toBeVisible();
...
test('responsive layouts and screenshots', async ({ page }, testInfo) => {
  ...
  const sizes = [[320,740],[360,800],[375,812],[390,844],[430,932],[768,1024],[1024,768],[1280,800],[1366,768],[1440,900],[1920,1080],[2560,1440]];
...
test('direct project routes, metadata and missing page', async ({ page }) => {
  for (const slug of ['allnrg','pdp','khasaut-tour','vpn-equipment','legacy-rheumatology','aurelia-atelier']) {
    const response = await page.goto(`/work/${slug}`);
    expect(response?.status()).toBe(200);
    await expect(page.locator('h1')).toHaveCount(1);
    await expect(page.locator('link[rel=canonical]')).toHaveAttribute('href', `https://1manakov.dev/work/${slug}`);
...
test('hero stays centered through resize, scroll and route return', async ({ page }) => {
  ...
  for (const [width, height] of [[1884,856], [1366,768], [1920,1080], [2560,1440], [1440,900]]) {
    await page.setViewportSize({ width, height });
    ...
    const geometry = await page.locator('.hero').evaluate(hero => {
      ...
      return { center: (title.y + title.height / 2 - rect.y) / rect.height,
        ...
        covered: surface.width >= parseFloat(getComputedStyle(hero).getPropertyValue('--lens-size')) || surface.width > 200 };
    });
...
test('rapid route activation, resize and failed image recovery', async ({ page }) => {
  await page.goto('/');
  await page.waitForTimeout(700);
  await page.locator('.lens-link').evaluate(el=>{el.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,button:0,detail:1}));el.dispatchEvent(new MouseEvent('click',{bubbles:true,cancelable:true,button:0,detail:1}));});
...
test('native touch lens swipe, tap and vertical scrolling', async ({ browser, browserName }) => {
  ...
  await expect(page.locator('.lens-link')).toHaveAttribute('data-project', 'pdp');
